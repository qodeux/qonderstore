import { Alert, Button, Input, Radio, RadioGroup, Select, SelectItem } from '@heroui/react'
...
const Checkout = () => {
  ...
  const { control, handleSubmit, watch, setValue, clearErrors, setError, formState } = checkoutForm
  ...
  const getShippingPrice = async (address: { postalCode: string; sublocality: number }) => {
    try {
      console.log(address)

      const { data } = await storeOrderService.getShippingPrice(address.sublocality)

      console.log('Costo de envio', data)
  ...
  useEffect(() => {
    if (cartItems.length === 0 && user?.role === 'customer') {
    ...
    >
      ...
      <div className='flex-grow'>
        ...
        <FormProvider {...checkoutForm}>
          <AddressFinder finderModule='checkout' isValidAddress={setHasMarker} onChangeAddress={getShippingPrice} />
        </FormProvider>
                ...
                description={
                  <div className='text-xs'>
                    {watchDeliveryType === 'standard' && (
                      <p>
                        Una vez que tu pago sea <strong>acreditado</strong>, procesaremos tu pedido y lo incluiremos en la próxima ruta de
                        entrega disponible, te enviaremos un mensaje de confirmación. Los tiempos de entrega pueden variar según la
                    ...
                    {watchDeliveryType === 'custom' && (
                      <p>
                        ...
                        enviaremos un mensaje de confirmación con los detalles de la entrega. Por favor, asegúrate de estar disponible en la
                        dirección proporcionada durante el horario seleccionado.
                      </p>
                    )}
                    {watchDeliveryType === 'express' && (
                      <p>
                        ...
                        entregas express pueden tener un <strong>costo adicional</strong> y están sujetas a la disponibilidad en tu área y a
                        nuestros <strong>horarios de rutas</strong>.
                      </p>
                    )}
                  </div>
                }
        ...
        <AnimatePresence>
          {cartItems.length !== 0 && (
            <header key='cart-header' className='px-4 py-2 flex items-center justify-between border-b border-foreground-400'>
              <h2 className='text-lg'>{`Resumen ${user?.role === 'customer' ? 'de tu' : 'del'} pedido`}</h2>
